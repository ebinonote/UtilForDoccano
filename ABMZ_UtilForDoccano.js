 ...
 * 
 * 
 * @command TagActor1Set
 * @text タグアクターの名前をセット
 * @desc アクターの名前をタグアクターの名前にします。
 * 
 * @arg ActorNameId
 * @text 名前用アクター
 * @desc このIDのアクターの名前を変更します。
 * @type actor
 * @default 6
 * 
 * @arg RPTag
 ...
 * 
 * 
 * @command TagActor1_2Set
 * @text タグアクターの名前を2つセット
 * @desc アクターの名前をタグアクターの名前にします。
 * 
 * @arg ActorNameId
 * @text 名前用アクター1ID
 ...
 * 
 *
 * @command RandomBattleMemberName
 * @text ランダムアクター
 * @desc バトルメンバーの中からランダムでアクターを決定し、
 * アクターの名前にセットします。
 * 
 * @arg ActorNameId
 * @text 名前用アクターID
...
//=============================================================================
    ...
    const pluginName = "ABMZ_UtilForDoccano";
	
	
    PluginManager.registerCommand(pluginName, "TagActor1Set", args => {
		let ActorNameId = args.ActorNameId;
		let RPTag = args.RPTag;
		let actorName = $gameParty.RPGTagActorName1(RPTag);
		if (actorName == "") {
			$gameActors.actor(ActorNameId).setName("");
			return;
		}
		$gameActors.actor(ActorNameId).setName(actorName);
    });
    PluginManager.registerCommand(pluginName, "TagActor1_2Set", args => {
		let ActorNameId = args.ActorNameId;
		let ActorNameId2 = args.ActorNameId2;
		let RPTag = args.RPTag;
		let actorName = $gameParty.RPGTagActorName1(RPTag);
		let actorName2 = $gameParty.RPGTagActorName2(RPTag, actorName);
		let actor = $gameActors.actor(ActorNameId);
		let actor2 = $gameActors.actor(ActorNameId2);
		if (actorName == "") {
			actor.setName("");
			actor2.setName("");
			return;
    ...
    PluginManager.registerCommand(pluginName, "RandomBattleMemberName", args => {
		let ActorNameId = args.ActorNameId;
		let TargetActorVariableId = args.TargetActorVariableId;
		let actor = $gameActors.actor(ActorNameId);
		let actorId = $gameParty.randomActorId();
		if (actorId == 0) {
			actor.setName("");
			$gameVariables.setValue(TargetActorVariableId, 0);
			return;
		}
		actor.setName($gameActors.actor(actorId).name());

		if (TargetActorVariableId == 0) {
			return;
		}
		$gameVariables.setValue(TargetActorVariableId, actorId);
	
    });
//=============================================================================
// ランダムで生存しているバトルアクターを入れる
//=============================================================================

Game_Party.prototype.randomActorId = function (){
	let battleMembers = this.battleMembers();
	let rand = Math.ceil(Math.random()*battleMembers.length);
	for (var i = 0; i < 10; i++) {
		let member = battleMembers[i];
...
//=============================================================================
// 装備のメタタグ<RPTag:勇気,ATK>のATKを返す
//=============================================================================

Game_Actor.prototype.RPTagPower = function (RPTagType){
	...
	for (let i = 0; i < equips.length; i ++) {
		...
		if (equip.meta.RPTag == null) {
			continue;
		}
		let RPTag = equip.meta.RPTag.split(",")[0];
		if (RPTag != RPTagType) {
			continue;
		}
		let param = equip.meta.RPTag.split(",")[1].toLowerCase();
		let paramId;
		switch (param) {
		case "hp": case "mhp":
			paramId = 0;
...
//=============================================================================
// 装備のメタタグ<RPTag:勇気,ATK>を装備しているATK1番目のアクターの名前
//=============================================================================

Game_Party.prototype.RPGTagActorName1 = function (RPTagType) {

	let power;
	let max = 0;
	let maxId = 0;
	let actor;
	let actors = this.members();
	for (let i = 0; i < actors.length; i++) {
		actor = actors[i];
		power = actor.RPTagPower(RPTagType);
		if (power > max) {
			max = power;
			maxId = actor.actorId();
			continue;
	...
	if (maxId == 0) return "";
	return $gameActors.actor(maxId).name();
}

Game_Party.prototype.RPGTagActorName2 = function (RPTagType, exceptActorName) {
	...
	for (let i = 0; i < actors.length; i++) {
		actor = actors[i];
		power = actor.RPTagPower(RPTagType);
		if (power > max && exceptActorName != actor.name()) {
			max = power;
			maxId = actor.actorId();
			continue;
		}
